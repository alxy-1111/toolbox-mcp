...
// ============================================================
server.registerTool("px_to_rem", {
    title: "px 单位换算",
    description: "把 px 换算成 rem 和 vw。rootFontSize 是根字号(默认16),designWidth 是设计稿宽度(默认375,用于算 vw)。",
    inputSchema: {
...
server.registerTool("color_convert", {
    title: "颜色格式转换",
    description: "输入任意颜色(如 #ff0000、rgb(255,0,0)、rgba(0,0,0,.5)),返回 hex / rgb / hsl 三种格式。",
    inputSchema: {
...
// ============================================================
function tsTypeOf(value, interfaces, nameHint) {
    if (value === null)
        return "null";
...
server.registerTool("json_to_ts", {
    title: "JSON 转 TS 接口",
    ...
    try {
        ...
        return textResult(`JSON 解析失败:${e.message}`);
    }
    const interfaces = [];
...
server.registerTool("npm_info", {
    ...
    try {
        ...
        try {
            const sizeRes = await fetchWithTimeout(`https://bundlephobia.com/api/size?package=${encoded}@${data.version}`, 6000);
            if (sizeRes.ok) {
                const size = (await sizeRes.json());
                const kb = (n) => (n ? `${(n / 1024).toFixed(1)} KB` : "未知");
        ...
        return textResult(`查询失败:${e.message}(可能是网络问题)`);
    }
});
// ============================================================
